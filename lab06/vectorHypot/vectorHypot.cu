 ...
 */
__global__ void vectorHypot(const float *A, const float *B, float *C, const unsigned long numElements)
{
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < numElements; ++i)
    {
        float hypot_sequential = sqrtf(h_A[i] * h_A[i] + h_B[i] * h_B[i]);
        if (fabs(hypot_sequential - h_C[i]) > ACCEPT_THRESHOLD)
        {
    ...
    for (int i = 0; i < numElements; ++i)
    {
        test_C[i] = sqrtf(h_A[i] * h_A[i] + h_B[i] * h_B[i]);
    }
