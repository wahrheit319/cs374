 ...
 */
__global__ void vectorAdd(const float *A, const float *B,  // input vecs
                          float *C,                        // output vec
                          const unsigned long numElements) // input size
{
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < numElements; ++i)
    {
        if (fabs(h_A[i] + h_B[i] - h_C[i]) > ACCEPT_THRESHOLD)
        {
